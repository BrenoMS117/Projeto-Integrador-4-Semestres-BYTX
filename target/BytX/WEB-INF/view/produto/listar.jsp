...
<head>
    ...
    <title>Listar Produtos - Sistema BytX</title>
...
<body>
    ...
    <div class="container">
        ...
        <div class="main-content">
            <div class="content-header">
                ...
                <% if (ehAdmin) { %>
                    <a href="${pageContext.request.contextPath}/produto/gerenciar?acao=novo" class="btn-primary">
            ...
            <div class="table-container">
                <div class="search-box">
                    <form id="searchForm" method="get" action="${pageContext.request.contextPath}/produto/listar">
                        ...
                        <c:forEach var="produto" items="${produtos}">
                            <tr>
                                ...
                                <td>
                                    ...
                                    <% if (ehAdmin) { %>
                                        <a href="${pageContext.request.contextPath}/produto/status?id=${produto.id}&acao=${produto.ativo ? 'desativar' : 'ativar'}"
                                           class="btn-action btn-status">
                                            <i class="fas ${produto.ativo ? 'fa-times' : 'fa-check'}"></i>
    ...
    <div class="footer">
        ...
        <p>Versão 2.3.1 | Módulo de Produtos</p>
    </div>

    <script>
        // Confirmação para ativar/desativar produtos
        document.addEventListener('DOMContentLoaded', function() {
            // Selecionar todos os links de status (ativar/desativar)
            const linksStatus = document.querySelectorAll('a.btn-status');

            linksStatus.forEach(link => {
                link.addEventListener('click', function(e) {
                    // Obter informações do produto da linha da tabela
                    const linha = this.closest('tr');
                    const idProduto = linha.cells[0].textContent;
                    const nomeProduto = linha.cells[1].textContent;
                    const acao = this.textContent.trim(); // "Ativar" ou "Desativar"
                    const isAtivacao = acao === 'Ativar';

                    // Mensagem personalizada
                    const mensagem = isAtivacao
                        ? `✅ Você está ativando o produto:\n\nO produto ficará visível na loja.`
                        : `❌ Você está desativando o produto:\n\nO produto NÃO ficará visível na loja.`;

                    // Pop-up de confirmação
                    const confirmacao = confirm(mensagem + "\n\nDeseja continuar?");

                    // Se cancelar, prevenir o clique
                    if (!confirmacao) {
                        e.preventDefault();
                        console.log('Ação cancelada pelo usuário');
                    } else {
                        console.log('Usuário confirmou a ação:', acao, 'Produto ID:', idProduto);
                    }
                });
            });
        });

        // Manter o script do menu igual ao dashboard
